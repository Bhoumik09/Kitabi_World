      ...
      >
        ...
        <div className="flex flex-col justify-between z-10 h-full text-xl">
          <div>
            ...
            <p className="mb-2">
              <strong>Language:</strong> {book?.language?.name.toUpperCase()}
            </p>
            {isAdmin && (
              <p className="mb-2">
                <strong>Pdf Link:</strong>{" "}
                <a href={`${book.book_link}`} className="underline">
                  BOOK
                </a>
              </p>
            )}
            {isAdmin && (
              <p className="mb-2">
                <strong>Sales:</strong> {book?.transactionCount || 0}
              </p>
            )}
            ...
            <div className="flex overflow-x-scroll gap-4 mb-2 no-scrollbar">
              {book?.tags?.map((tag, index) => (
                <div
                  key={index}
                  className="bg-blue-400 h-fit px-2 py-2 rounded-xl"
                >
                  {tag.name}
                </div>
              ))}
            ...
            <div className="flex overflow-x-scroll gap-4 mb-2 no-scrollbar">
              {book.authors.map((author, index) => (
                <div
                  key={index}
                  className="bg-green-400 h-fit px-2 py-2 rounded-xl"
                >
                  {author.name}
                </div>
              ))}
